...
Для того, чтобы добавить обработчик события на элемент, есть несколько
способов.

Первый - добавить атрибут on* (вместо звездочки подставить имя
события)
...
- По факту, **событие click вмещает в себя два события - mousedown и
mouseup**. Выполняются они в таком порядке: `mousedown`, `mouseup`, `click`.
